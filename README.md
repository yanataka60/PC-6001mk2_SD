...
## MODE5への対応テスト(本体BASIC-ROMの差し替えが必要です、自己責任でお願いします)
　MODE5では裏RAMが使われるため、「BASIC-ROMを裏RAMにコピーしCMT関連ルーチンをSD-CARDアクセスに書き換え」が出来ませんのでSD-CARDアクセス用にパッチを当てたBASIC-ROMに差し替えることでMODE5でのSD-CARD対応テストをしています。
...
　本体BASIC-ROMの位置及び吸出しに関しても自分で解決できる方のみ挑戦願います。

### Arduinoプログラム
　MODE5(Caution)フォルダ、ArduinoフォルダのPC-6001mk2_SDフォルダ内PC-6001mk2_SD.inoを書き込みます。

MODE1～MODE4用のプログラムにMODE5用のプログラムが付加されていますのでこちらを書き込めばMODE1～MODE4用のプログラムに戻す必要はありません。

### ROMへの書込み
　MODE5(Caution)フォルダ、Z80フォルダ内のEXT_ROM.binをROMライター(TL866II Plus等)を使って2764又は28C64に書き込みます。

　こちらもMODE1～MODE4用のプログラムにMODE5用のプログラムを付加したものです。MODE5対応を気にしなければMODE1～MODE4用のプログラムに戻す必要はありません。
...
|ADDRESS|修正前|修正後|
| ------------ | ------------ | ------------ |
|1A61|C3 12 58|C5 D5 E5|
|1A70|C3 15 58|C5 D5 E5|
|1AB8|C3 18 58|C5 D5 E5|
|1ACC|C3 1B 58|C5 D5 E5|
|1B06|C3 1E 58|C5 D5 E5|

　MODE5(Caution)フォルダ、Z80フォルダ内のROM_PATCH.binの内容で5812H～5A20Hまでを差し替えます。

　この差し替えによりfilesが使えなくなるはずですが、FDDを繋いでいない限り問題は発生しないと思います。
